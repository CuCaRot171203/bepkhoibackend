...
namespace BepKhoiBackend.BusinessObject.Services.OrderService
{
    public class OrderService : IOrderService
    {
        ...
        public async Task<DeliveryInformationDto?> GetDeliveryInformationByIdAsync(int deliveryInformationId)
        {
            try
            {
                var deliveryInfo = await _orderRepository.GetDeliveryInformationByIdAsync(deliveryInformationId);
                if (deliveryInfo == null)
                {
                    return null;
                }
            ...
            {
                throw new Exception($"Error retrieving DeliveryInformation: {ex.Message}", ex);
            }
        }
    }
}
